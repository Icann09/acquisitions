...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    if (decision.isDenied() && decision.reason.isShield()) {
      logger.warn('Shiled blocked request', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path, method: req.method });
    ...
    res.status(500).json({ error: 'Internal server error', message: 'Something went worng with security middleware' });
  }
};
